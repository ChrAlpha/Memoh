...
// TestProvideAgentWiresLoopReselectMode exercises provideAgent's config ->
// native.Deps wiring end to end through the only exported observation point
// (LoopReselectMode()); it also wires ContextViewApplier from the same Deps
// literal, which has no exported getter to assert on directly here.
