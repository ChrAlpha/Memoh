...
function deferred() {
  let resolve!: (value: { data: { revision: string } }) => void
  let reject!: (error: unknown) => void
  const promise = new Promise<{ data: { revision: string } }>((yes, no) => {
    resolve = yes
    reject = no
  })
...
describe('file viewer save snapshot', () => {
  ...
  it('keeps undoing to the old baseline dirty after the submitted text is saved', async () => {
    const view = await mountViewer()
    await view.edit('A')
    const pending = deferred()
    sdk.write.mockReturnValueOnce(pending.promise)
