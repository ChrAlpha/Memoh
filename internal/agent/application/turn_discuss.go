...
func (s *Service) pumpDiscussNative(ctx context.Context, cmd turn.StartTurnCommand, h *discussHandle, resolved ResolveRunConfigResult) {
	...
	runConfig.Query = ""
	// ResolveRunConfig returns the context budget as a sibling result field,
	// not on the inner RunConfig (buildBaseRunConfig has no Messages yet to
	// size a budget against) — mirrors resumeAgentSession's identical fixup.
	...
	// prepareRunConfig already populated ContextSourceFrags from the base
	// (message-less) RunConfig, so it carries only system/memory/hook
	// fragments. The discuss turn content above is transport-flattened
	// (turn.DiscussMessage), not the RC/TR streams the contextview discuss
	// collector needs to rebuild fragments from, so clear the stale source
	// frags rather than ship a provider view missing the conversation:
	// ApplyProviderRunConfig treats non-empty ContextSourceFrags as
	// authoritative and never falls back to cfg.Messages. Clearing them
	// routes discuss through ApplyProviderRunConfig's per-field collector
	// path instead, which reads cfg.Messages/cfg.System directly.
