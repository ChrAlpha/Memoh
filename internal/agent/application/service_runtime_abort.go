...
//nolint:contextcheck // lifecycle reconciliation intentionally outlives the abort acknowledgement.
	...
	for {
		...
		case runErr == nil:
			...
			if state == ledger.StateAborted {
				...
				_, currentPending, snapshotErr := s.PendingRuntimeDecision(ctx, runID)
				if snapshotErr != nil {
					s.recordContextLifecyclePersistenceError(
						snapshotErr,
						runID,
