...
func TestPrepareContinuationRunConfigReplacesStaleContextAndSetsCapabilities(t *testing.T) {
	...
	if got.ContextCurrentUserMessageIndex != nil {
		t.Fatalf("ContextCurrentUserMessageIndex = %d, want nil after continuation replaces Messages", *got.ContextCurrentUserMessageIndex)
	}
	if got.ContextToolExchangePolicy == nil || got.ContextToolExchangePolicy.MinMessages != 10 {
		t.Fatalf("ContextToolExchangePolicy = %+v, want default policy", got.ContextToolExchangePolicy)
	}
	if got.ContextTrimmableMessages != len(got.Messages) || len(got.ContextHistoryTokenEstimates) != len(got.Messages) {
		t.Fatalf("budget-trim signals = trimmable:%d estimates:%d, want both %d", got.ContextTrimmableMessages, len(got.ContextHistoryTokenEstimates), len(got.Messages))
	}
}

func TestPrepareContinuationRunConfigPreservesExistingToolExchangePolicy(t *testing.T) {
	t.Parallel()

	existing := &contextfrag.ToolExchangePolicy{MinMessages: 3}
	got, err := (&Service{}).prepareContinuationRunConfig(
		context.Background(),
		native.RunConfig{ContextToolExchangePolicy: existing},
		historyfrag.ScopeFallback{},
		contextfrag.Scope{},
		nil,
	)
	...
	if got.ContextToolExchangePolicy != existing {
		t.Fatalf("ContextToolExchangePolicy = %+v, want untouched existing pointer", got.ContextToolExchangePolicy)
	}
}
