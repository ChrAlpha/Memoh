...
func TestDBMessageSourceHashTracksPersistedTurnDAGFields(t *testing.T) {
	...
	first, err := FromDBMessage(base, ScopeFallback{})
	if err != nil {
		t.Fatalf("first FromDBMessage failed: %v", err)
	}
	second, err := FromDBMessage(base, ScopeFallback{})
	if err != nil {
		t.Fatalf("second FromDBMessage failed: %v", err)
	}
...
func TestDBMessageSourceHashTracksPersistedPayloadFields(t *testing.T) {
	...
	changedContent := base
	changedContent.Content = persistedModelMessage(t, turn.ModelMessage{Role: "user", Content: turn.NewTextContent("hello again")})
...
func TestModelMessageToSDKPreservesTopLevelToolCalls(t *testing.T) {
	...
	mm := turn.ModelMessage{
		Role:    "assistant",
		Content: turn.NewTextContent("I will look that up."),
...
func TestModelMessageToSDKPreservesTopLevelToolResultName(t *testing.T) {
	...
	mm := turn.ModelMessage{
		Role:       "tool",
		Content:    turn.NewTextContent("42"),
		ToolCallID: "call-42",
		Name:       "calculator",
	}
