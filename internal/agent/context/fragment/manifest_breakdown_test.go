...
func TestBuildManifestItemsCarryTokenEstimates(t *testing.T) {
	...
	for _, item := range manifest.Items {
		if want, ok := wantByID[item.ID]; ok {
			if item.TokenEstimate != want {
...
func TestLifecycleSnapshotCarriesBreakdown(t *testing.T) {
	t.Parallel()

	manifest := BuildManifest(breakdownFixtureFrags())
	snapshot := BuildLifecycleSnapshot(manifest)
	if len(snapshot.Breakdown) != len(manifest.Breakdown) {
		t.Fatalf("snapshot breakdown = %+v, want %+v", snapshot.Breakdown, manifest.Breakdown)
	}
	for i := range manifest.Breakdown {
		if snapshot.Breakdown[i] != manifest.Breakdown[i] {
			t.Fatalf("snapshot breakdown[%d] = %+v, want %+v", i, snapshot.Breakdown[i], manifest.Breakdown[i])
		}
	}
	if snapshot.Counts.TokenEstimate != manifest.Counts.TokenEstimate {
		t.Fatalf("snapshot counts token estimate = %d, want %d", snapshot.Counts.TokenEstimate, manifest.Counts.TokenEstimate)
	}
}

func TestLifecycleHolderClonesBreakdown(t *testing.T) {
	t.Parallel()

	manifest := BuildManifest(breakdownFixtureFrags())
	holder := NewLifecycleHolder()
	holder.SetManifest(manifest)
	manifest.Breakdown[0].TokenEstimate = -1
	snapshot, ok := holder.Snapshot()
	if !ok {
		t.Fatal("holder must produce a snapshot")
	}
	if snapshot.Breakdown[0].TokenEstimate == -1 {
