...
func BuildUserMessageMetaFromInput(input UserMessageHeaderInput) UserMessageMeta {
	...
	meta := UserMessageMeta{
		...
		ForwardedFrom: forwardedFromValue(input.ForwardSender, input.ForwardFromUserID, input.ForwardFromConversationID),
	}
...
// forwardedFromValue mirrors the timeline rendering fallback order for a
// forwarded message's origin label without importing the timeline package.
