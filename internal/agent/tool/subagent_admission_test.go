...
func (f *fakeSubagentAdmitter) AdmitSubagentRun(ctx context.Context, botID, threadID, invocationID string, submission []byte) (context.Context, string, func(error), error) {
	f.mu.Lock()
	if f.reject != nil {
		err := f.reject
		f.mu.Unlock()
	...
	return runCtx, runID, func(cause error) {
		defer cancel()
		f.mu.Lock()
		defer f.mu.Unlock()
