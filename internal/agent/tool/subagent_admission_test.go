...
func (f *fakeSubagentAdmitter) AdmitSubagentRun(ctx context.Context, botID, threadID, invocationID string, submission []byte) (context.Context, string, func(SubagentTerminal), error) {
	f.mu.Lock()
	if f.reject != nil {
		err := f.reject
		f.mu.Unlock()
	...
	return runCtx, runID, func(terminal SubagentTerminal) {
		defer cancel()
		f.mu.Lock()
		defer f.mu.Unlock()
...
func TestSpawnedTurnReusesAdmittedRunIDAndFinishesOnceAfterRetries(t *testing.T) {
	first := &contextfrag.LifecycleSnapshot{
		Version: 1,
	...
	if !reflect.DeepEqual(terminals[0].contextLifecycle, final) {
		t.Fatalf("terminal snapshot = %#v, want final retry snapshot %#v", terminals[0].contextLifecycle, final)
	}
}
