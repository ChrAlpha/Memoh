...
type SubagentAdmitter interface {
	// AdmitSubagentRun returns the context and stable run identity the run must
	// execute with, plus the terminal write that releases the thread's slot.
	// turn.ErrSessionBusy means the agent is already working;
	// turn.ErrDuplicateTurn means this task already has a run.
