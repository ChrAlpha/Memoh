...
// ToolDefBucket aggregates tool definition estimates per provider bucket
// ("native", "mcp") from the latest turn's lifecycle snapshot.
...
func (h *SessionInfoHandler) GetSessionInfo(c echo.Context) error {
	...
	if rows, err := h.queries.ListRecentAssistantMessagesBySession(ctx, sqlc.ListRecentAssistantMessagesBySessionParams{
