...
func TestAggregateContextLifecycleCacheEfficiency(t *testing.T) {
	...
	if agg.TotalExpectedStableTokens != 80000 {
		t.Fatalf("expected stable tokens = %d, want 80000 (first observation excluded)", agg.TotalExpectedStableTokens)
	}
...
func TestAggregateContextLifecycleToolRosterChurn(t *testing.T) {
	...
	agg := aggregateContextLifecycle(turns)
	if agg.ToolRosterChanges != 2 {
		t.Fatalf("roster changes = %d, want 2", agg.ToolRosterChanges)
	}
	if len(agg.ToolRosterChangeDetails) != 2 {
