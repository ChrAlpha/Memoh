...
func TestHookSystemSectionsDropOptionalBeforePreferredWithNotice(t *testing.T) {
	...
	rendered := renderSDK(t, result.Selected, placementFor(result.Selected))
	if !strings.Contains(rendered.System, optional.ID) {
		t.Fatalf("system budget marker = %q, want omitted hook ID", rendered.System)
	}
}
