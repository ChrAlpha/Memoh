...
func TestDiscussCollectorProviderIntentRepairsDanglingToolCalls(t *testing.T) {
	...
	frags := collectDiscussProviderContext(t, []pipeline.ContextMessage{
		{
			Role:       "assistant",
...
func TestDiscussCollectorProviderIntentDropsOrphanToolResults(t *testing.T) {
	...
	if msg == nil || !sdkMessagesJSONEqual(*msg, sdk.UserMessage("next question")) {
		t.Fatalf("remaining message = %#v, want current user", msg)
	}
}

func collectDiscussProviderContext(t *testing.T, messages []pipeline.ContextMessage) []contextfrag.ContextFrag {
	t.Helper()
