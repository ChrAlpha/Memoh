...
func latestUserIndex(tagged []TaggedFrag) int {
	for i := len(tagged) - 1; i >= 0; i-- {
		if tagged[i].Frag.Role == sdk.MessageRoleUser {
			return i
		}
		for _, part := range tagged[i].Frag.Parts {
			if msg := sdkMessagePart(part); msg != nil && msg.Role == sdk.MessageRoleUser {
				return i
			}
		}
	}
...
func isToolResultFrag(frag contextfrag.ContextFrag) bool {
	if frag.Role == sdk.MessageRoleTool {
		return true
	}
	for _, part := range frag.Parts {
		if msg := sdkMessagePart(part); msg != nil && msg.Role == sdk.MessageRoleTool {
			return true
		}
	}
	return false
...
func isToolExchangeFrag(frag contextfrag.ContextFrag) bool {
	if isToolResultFrag(frag) {
		return true
	}
	for _, part := range frag.Parts {
		if partHasToolExchange(part) {
			return true
		}
		if msg := sdkMessagePart(part); msg != nil && messageHasToolExchange(*msg) {
			return true
		}
	}
	return false
...
func messageHasToolExchange(msg sdk.Message) bool {
...
func partHasToolExchange(part contextfrag.Part) bool {
	if strings.Contains(string(part.Type), "tool-call") ||
		strings.Contains(string(part.Type), "tool_call") ||
		strings.Contains(string(part.Type), "tool-result") ||
		strings.Contains(string(part.Type), "tool_result") {
		return true
	}
	return false
}
