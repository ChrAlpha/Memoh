...
func TestSelectCompactionRecordsMatchesGuardedSweep(t *testing.T) {
	t.Parallel()
	...
	if got := candidateIDs(toCompact); len(got) != 2 || got[0] != "loop-1" || got[1] != "loop-2" {
		t.Fatalf("selected = %#v, want guarded current-turn middle", got)
	}
}
